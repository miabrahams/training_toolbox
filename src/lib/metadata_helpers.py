...
def comfy_nodes(comfy_prompt: dict[str, Any]) -> dict[str, dict[str, Any]]:
    return {id: value for (id, value) in comfy_prompt.items()}

def comfy_unique_node_types(comfy_prompt: dict[str, Any]) -> list[str]:
    node_types = [value['class_type'] for value in comfy_prompt.values()]
...
# Extract the artist name from positive prompt
...
def artist_from_prompt(prompt: str, method=0):
    if method == 0:
